...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      init(container: HTMLElement, useWin?: boolean) {
        this.container = container;
        this.useWindow = !!useWin;
        const target: any = this.useWindow ? window : container;
      ...
      setCoords(x: number, y: number) {
        ...
        const nx = ((x - left) / w) * 2 - 1;
        const ny = -(((y - top) / h) * 2 - 1);
      ...
      onDocumentMouseMove(event: MouseEvent) {
        ...
        if (this.isAutoActive && !this.hasUserControl && !this.takeoverActive) {
          if (!this.container && !this.useWindow) return;
          const w = this.useWindow ? (window.innerWidth || 1) : (this.container!.getBoundingClientRect().width || 1);
          const h = this.useWindow ? (window.innerHeight || 1) : (this.container!.getBoundingClientRect().height || 1);
...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any) {
        ...
        Mouse.init(props.$wrapper, props.listenOnWindow);
